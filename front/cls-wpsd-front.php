...
class Wpsd_Front
{
    ...
    public function wpsd_front_assets()
    {
        ...
        wp_enqueue_script('google-recaptcha3-js', 'https://www.google.com/recaptcha/api.js');
        //
        wp_enqueue_script(
    ...
    private function wpsd_check_google_recaptcha($data)
    {
        ...
        if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
            if (!isset($data['g-recaptcha-response'])) {
                $data['g-recaptcha-response'] = $_POST['g-recaptcha-response'];
            }
        }
        if (isset($data['wpsd_token']) && !empty($data['wpsd_token'])) {
            if (!isset($data['g-recaptcha-response'])) {
                $data['g-recaptcha-response'] = $data['wpsd_token'];
            }
        }
        if (!isset($data['g-recaptcha-response'])) {
            $result = ['status' => false, 'message' => 'Empty g-recaptcha-response value: can`t validate captcha.'];
            return $result;
        }
        // @see: https://developers.google.com/recaptcha/docs/verify
        $postdata = http_build_query([
            'secret' => '6Ld-pa8kAAAAAJ2y4XrA9mVW3sljvYcSzmQ5pK30',
